...
# ========= 検出（ゆるめ：GOGOGO出やすく）=========
TH1_DET, TH2_DET = 8, 15      # 差分しきい（小さめ）
MIN_AREA_DET = 80             # 最小面積（小さめ）

# ========= トラック & 連結距離 =========
PERSIST = 5
MAX_LINK_DIST = 36

# ========= 簡易スタビライズ（整数シフト：NumPy）=========
DOWNSCALE, SEARCH = 4, 3  # 1/4縮小で±3px探索（SAD最小）
...
MIN_GO_AREA_STRONG = 750        # GOの面積しきい（緩め）
WIN_N, WIN_M = 5, 2             # 多数決：直近5フレ中2回でGO
GO_OFF_DECAY = 2                # 未検出時の減衰

# ---- Temporal Accumulator（時間方向の“溜まり”）----
# motion (0/255) を蓄積。弱い連続動作でも面積が伸びてGOしやすくなる
TEMP_DECAY = 0.80               # 0.80なら1フレ毎に20%減衰
TEMP_THR   = 60                 # Acc > 60 を“有効”とみなす（0..255想定）

# ---- ブレ連動しきい（大ブレ時だけ厳しく）----
SHAKE_THR       = 4             # |dx|+|dy| >= 4 で大ブレ判定
SHAKE_ADD_TH    = 4             # ブレ時のしきい上乗せ値
SHAKE_ADD_AREA  = 40            # ブレ時の面積しきい上乗せ

# ---- トラック基準のGO（面積が小さくても本当に動いてればGO可）----
V_MIN = 18.0                    # px/s（平均速度がこれ以上のトラックがあればGOに加点）
TRACK_GO_MIN_PERSIST = 4        # 最低継続フレーム
...
USE_CV_DRAW = True  # FalseにするとNumPyで簡易枠のみ（テキスト無し）
...
def main():
    ...
    # Temporal Accumulator（0..255, float32）
    motion_acc = np.zeros((H, W), np.float32)
    ...
    while True:
        ...
        # ===== Temporal Accumulator で“溜まり”を作る =====
        # macc = max( decay*macc, motion )
